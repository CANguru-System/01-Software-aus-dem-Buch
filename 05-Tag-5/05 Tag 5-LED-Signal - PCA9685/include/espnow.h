...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *data, int data_len)
{
  ...
  case SWITCH_ACC:
  {
    ...
    uint16_t _to_address = (uint16_t)((opFrame[7] << 8) | opFrame[8]);
    for (int signal = 0; signal < num_signals; signal++)
    {
      // Auf benutzte Adresse überprüfen
      if (_to_address == Signals[signal].Get_to_address())
      {
        Signals[signal].SetLightDest((colorLED)opFrame[9]);
        // muss Artikel geändert werden?
        if (Signals[signal].ColorChg())
        {
          switchAcc(signal);
        }
        break;
